...
public:
  ...
  bool isEmpty() const { return (N == 0 && S.empty()); }
};
...
public:
  ...
  friend llvm::raw_ostream &operator<<(llvm::raw_ostream &Out,
                                       const SymbolicCountPrintWrap &SCW) {
    auto &SC = SCW.SC;
...
bool operator==(const SymbolicCount &L, const SymbolicCount &R) {
  if (L.N < 0 || L.N != R.N || L.S.size() != R.S.size())
    return false;
  bool eq = true;
  auto itL = L.S.begin();
  auto itR = R.S.begin();
  for (int i = 0; i < L.S.size(); i++) {
    auto eL = *itL;
    auto eR = *itR;
    // TODO: this only checks that the IRExprs have the same pointer, so it is
    // more restrictive than necessary need to add equality to IRExpr itself
...
// a normal IRVarRef shouldn't be misaligned so we keep the LSB as 1 for sanity
// check
...
private:
  ...
  IRExpr *extractLoopCount(LoopIRStmt *LS, EnvTy &PreEnv, EnvTy &PostEnv) {
    if (auto CondE = dyn_cast<BinopIRExpr>(LS->Cond.get())) {
      ...
      switch (CondE->Op) {
      case BinopIRExpr::BINOP_GE: {
        IterOfs = 1;
      };
      case BinopIRExpr::BINOP_GT: {
        IterSel = CondE->Right.get();
        break;
      }
      case BinopIRExpr::BINOP_LE: {
        IterOfs = 1;
      }
      case BinopIRExpr::BINOP_LT: {
        IterSel = CondE->Left.get();
        break;
      }
      default: {
      }
      ...
      if (!IncS || IncS->Op != BinopIRExpr::BINOP_ADD)
        return nullptr;
      ...
      if (VARREF_TO_SYMVAR(IncL->Ident) != PreEnv[IterVar->Ident])
        return nullptr;

      // TODO: add something for when the initialization is not 0
      auto *IncE = IncS->Right.get()->clone();
      auto *IterOpp = ((IterSel == CondE->Left.get()) ? CondE->Right.get()
                                                      : CondE->Left.get())
                          ->clone();
  ...
  void addToEnv(EnvTy &Env, IRVarRef Dest, IRExpr *Src) {
    // TODO: this analysis is unsound when the variable is aliased to something
    // else you could have code like int i = 0; (v0) i += 1     (v1) y = &i; *y
    // = -1;   (i |-> v1)
    // <- here the environment would say that i -> v1 = v0 + 1, when in fact i =
    // -1
    auto *SrcC = Src->clone();
  ...
  void countSpawns(EnvTy &Env, SymbolicCount *CDest, IRBasicBlock *B) {
    assert(CDest);
    ...
    if (B->Term) {
      if (auto *IS = dyn_cast<IfIRStmt>(B->Term)) {
        ...
        assert((empty || JoinB) && "Closure declaration before if/else without "
                                   "join.. where would the spawn next go?");
        ...
        if (JoinB != B->Succs[1]) { // have a real else branch
          ...
          // If the if and else branches spawn the same amount, then we know the
          // overall spawn count.
          if (*IfCnt == *ElseCnt) {
            determined = true;
            *CDestL += *IfCnt;
          ...
          // If there are no spawns in this if, it doesnt matter that we cannot
...
public:
  SpawnCounter() { InvalidCount = new SymbolicCount(0); }
  ~SpawnCounter() {
    delete InvalidCount;
...
void CountSpawns(IRProgram &P, ASTContext &C) {
  auto IC = IRPrintContext{.ASTCtx = C, .NewlineSymbol = "\n"};
  for (auto &F : P) {
    SpawnCounter SC;
