...
void DAEAtInd(IRProgram &P, IRBasicBlock *B, int Ind) {
  ...
  if (!AccessStmt) {
    PANIC("DAE annotation should be proceeded by a copy IR statement "
          "(decouplable access)");
  }

  IRFunction *LoadF = P.createFunc(B->getParent()->getName() + "_dae_" +
                                       std::to_string(DaeCount++),
                                   AccessStmt->Dest->Type);
