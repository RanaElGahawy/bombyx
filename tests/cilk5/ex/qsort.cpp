...
THREAD(sample_qsort) {
    ...
    if ((largs->begin != largs->end)) {
        (--largs->end);
        middle = partition(largs->begin,largs->end,bind2nd(std::less<int>(),*(largs->end)));
        swap(*(largs->end),*(middle));
...
THREAD(qmain_cont0) {
    ...
    for (i0 = 0;(i0 < (largs->n - 1));(++i0)) {
        if (((largs->a[i0] >= largs->a[(i0 + 1)]) || (largs->a[i0] != i0))) {
            std::cerr << "Sort failed at location i=" << i << " a[i] = " << a[i] << " a[i+1] = " << a[i + 1] << std::endl;
            delete [] a;
