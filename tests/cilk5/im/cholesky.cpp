 ...
 */
static InternalNode *new_block_leaf(void) {
 ...
 */
static InternalNode *new_internal(InternalNode *a00, InternalNode *a01,
  ...
  if (depth == BLOCK_DEPTH) {
    ...
    if (r < mid) {
      if (c < mid)
        ...
        a->child[_01] = set_matrix(depth, a->child[_01], r, c - mid, value);

    } else {
      if (c < mid)
        a->child[_10] = set_matrix(depth, a->child[_10], r - mid, c, value);
      else
        a->child[_11] =
...
Matrix mul_and_subT(int depth, int lower, Matrix a, Matrix b, Matrix r) {
  ...
  if (a->child[_00] && b->child[TR_00]) {
    r00 = cilk_spawn mul_and_subT(depth, lower, a->child[_00], b->child[TR_00],
                                  r00);
  }

  if (!lower && a->child[_00] && b->child[TR_01]) {
    r01 =
        cilk_spawn mul_and_subT(depth, 0, a->child[_00], b->child[TR_01], r01);
  }

  if (a->child[_10] && b->child[TR_00]) {
    r10 =
        cilk_spawn mul_and_subT(depth, 0, a->child[_10], b->child[TR_00], r10);
  }

  if (a->child[_10] && b->child[TR_01]) {
    r11 = cilk_spawn mul_and_subT(depth, lower, a->child[_10], b->child[TR_01],
                                  r11);
  }
  cilk_sync;

  if (a->child[_01] && b->child[TR_10]) {
    r00 = cilk_spawn mul_and_subT(depth, lower, a->child[_01], b->child[TR_10],
                                  r00);
  }

  if (!lower && a->child[_01] && b->child[TR_11]) {
    r01 =
        cilk_spawn mul_and_subT(depth, 0, a->child[_01], b->child[TR_11], r01);
  }

  if (a->child[_11] && b->child[TR_10]) {
    r10 =
        cilk_spawn mul_and_subT(depth, 0, a->child[_11], b->child[TR_10], r10);
  }

  if (a->child[_11] && b->child[TR_11]) {
    r11 = cilk_spawn mul_and_subT(depth, lower, a->child[_11], b->child[TR_11],
                                  r11);
  }
  cilk_sync;
