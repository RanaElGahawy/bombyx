...
THREAD(fun_reentry0) {
  ...
  if ((largs->iter < largs->rounds)) {
    ...
    spawn_next<fun_reentry0_cont0_closure> SN_fun_reentry0_cont0(
        SN_fun_reentry0_cont0c);
    cont sp0k;
...
THREAD(fun_reentry0_cont0) {
  fun_reentry0_cont0_closure *largs =
      (fun_reentry0_cont0_closure *)(args.get());
