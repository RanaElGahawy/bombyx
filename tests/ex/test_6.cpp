...
CLOSURE_DEF(magnitude, Vec2 *v;);
CLOSURE_DEF(compute, Vec2 *a; Vec2 * b;);
CLOSURE_DEF(compute_cont0, float m1; float m2;);
CLOSURE_DEF(main_cont0, float result;);

THREAD(magnitude) {
  magnitude_closure *largs = (magnitude_closure *)(args.get());
  SEND_ARGUMENT(largs->k,
                ((largs->v->x * largs->v->x) + (largs->v->y * largs->v->y)));
  return;
}
THREAD(compute) {
  float m1;
